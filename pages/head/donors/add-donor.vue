...
import { generatePassword as passGen } from '~/helpers/app.helpers'
  ...
  computed: {
    coordinatorOptions() {
      return this.$store.state.head.coordinators.map((coordinator) => {
        return {
          text: `${coordinator.user.personalInfo.firstName} ${coordinator.user.personalInfo.middleName} ${coordinator.user.personalInfo.lastName}`,
    ...
    donorUserOptions() {
      return this.$store.state.head.donorUsers.map((user) => {
        return {
          text: `${user.personalInfo.firstName} ${user.personalInfo.middleName} ${user.personalInfo.lastName}`,
          value: user.id,
        }
      })
    },
